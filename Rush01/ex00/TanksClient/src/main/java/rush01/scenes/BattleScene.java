...
import rush01.Main;
import rush01.objects.bulletLogic;
...
public class BattleScene implements mainConstants {
    ...
    private static void draw(Pane root, World data) {
        ...
        if (player.tank.id == 1) {
            for (bulletLogic bullet : player.bullets) {
                ImageView img = new ImageView(bulletLogic.PLAYER_BULLET);
                img.setX(bullet.getX() + Tank.imageWidth / 2);
                img.setY(TANK_OFFSET_Y - player.tank.getY() - bullet.getY());
                img.setFitHeight(bulletLogic.imageHeight);
                img.setFitWidth(bulletLogic.imageWidth);
        ...
        if (enemy.tank.id == 1) {
            for (bulletLogic bullet : enemy.bullets) {
                ImageView img = new ImageView(bulletLogic.ENEMY_BULLET);
                img.setX(bullet.getX());
                img.setY(bullet.getY());
                img.setFitHeight(bulletLogic.imageHeight);
                img.setFitWidth(bulletLogic.imageWidth);
                root.getChildren().add(img);
